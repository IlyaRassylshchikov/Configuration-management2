...
Этапы 1-5: CLI-приложение с визуализацией на Mermaid.
"""
...
class DependencyVisualizer:
    ...
    def __init__(self, package_name: str, repo_url: str, 
                 ...
                 max_depth: int = 10, output_mermaid: bool = False,
        ...
        Args:
            ...
            output_mermaid: Вывести граф в формате Mermaid
            mermaid_file: Путь к файлу для сохранения Mermaid диаграммы
        """
        ...
        self.output_mermaid = output_mermaid
    ...
    def fetch_package_info_npm(self, package_name: str) -> Dict:
        ...
        try:
            ...
            req = urllib.request.Request(
                url,
                headers={
                    'User-Agent': 'DependencyVisualizer/0.5.0',
    ...
    def generate_mermaid(self) -> str:
        ...
        lines.append("    %% Определение стилей")
        lines.append("    classDef rootNode fill:#e1f5ff,stroke:#01579b,stroke-width:3px")
        lines.append("    classDef normalNode fill:#f5f5f5,stroke:#666,stroke-width:2px")
        lines.append("    classDef cyclicNode fill:#ffebee,stroke:#c62828,stroke-width:2px")
        lines.append("")
        ...
        lines.append("    %% Определение узлов")
        for node in sorted(self.graph.nodes):
            safe_node = node.replace("-", "_").replace("@", "_").replace("/", "_")
            label = node
            
            if node == self.package_name:
        ...
        # Добавляем рёбра
        lines.append("    %% Определение связей")
        for from_node in sorted(self.graph.nodes):
            deps = sorted(self.graph.get_dependencies(from_node))
            for to_node in deps:
                safe_from = from_node.replace("-", "_").replace("@", "_").replace("/", "_")
                safe_to = to_node.replace("-", "_").replace("@", "_").replace("/", "_")
                lines.append(f"    {safe_from} --> {safe_to}")
        
        lines.append("")
        
        # Применяем стили
        lines.append("    %% Применение стилей")
        root_safe = self.package_name.replace("-", "_").replace("@", "_").replace("/", "_")
        lines.append(f"    class {root_safe} rootNode")
        
        if cyclic_nodes:
            cyclic_safe = [n.replace("-", "_").replace("@", "_").replace("/", "_") for n in cyclic_nodes]
            lines.append(f"    class {','.join(cyclic_safe)} cyclicNode")
        
        return "\n".join(lines)
    
    def print_mermaid(self) -> None:
        """Вывод Mermaid диаграммы на экран."""
        print(f"\n{'=' * 60}")
        print(f"ВИЗУАЛИЗАЦИЯ ГРАФА ЗАВИСИМОСТЕЙ (Mermaid)")
        ...
        print(f"\n{'=' * 60}")
        print(f"Всего узлов: {len(self.graph.nodes)}")
        total_edges = sum(len(deps) for deps in self.graph.edges.values())
        print(f"Всего связей: {total_edges}")
        ...
        print("2. Откройте https://mermaid.live")
        print("3. Вставьте код в редактор")
        
        if self.mermaid_file:
            print(f"\n✓ Диаграмма сохранена в файл: {self.mermaid_file}")
        
        print(f"{'=' * 60}\n")
    
    def save_mermaid(self) -> None:
        """Сохранение Mermaid диаграммы в файл."""
        if not self.mermaid_file:
            return
        
        mermaid_code = self.generate_mermaid()
        
        try:
            with open(self.mermaid_file, 'w', encoding='utf-8') as f:
                f.write(f"# Граф зависимостей для пакета: {self.package_name}\n\n")
                f.write(f"Дата создания: 2025-11-23\n")
                f.write(f"Всего узлов: {len(self.graph.nodes)}\n")
                total_edges = sum(len(deps) for deps in self.graph.edges.values())
                f.write(f"Всего связей: {total_edges}\n\n")
                f.write("```mermaid\n")
                f.write(mermaid_code)
                f.write("\n```\n")
                
            print(f"✓ Mermaid диаграмма сохранена в файл: {self.mermaid_file}")
    ...
    def analyze_dependencies(self) -> None:
        ...
        try:
            ...
            # Вывод в формате Mermaid (Этап 5)
            if self.output_mermaid:
                self.print_mermaid()
                if self.mermaid_file:
                    self.save_mermaid()
            else:
                # Вывод графа в текстовом формате
                self.print_graph()
...
Примеры использования:
  ...
  # Визуализация в формате Mermaid
  %(prog)s -p A -r ./test_data/simple_graph.json -t --mermaid
  
  # Сохранение диаграммы в файл
  %(prog)s -p A -r ./test_data/simple_graph.json -t -m -o graph.md
  
  # Исключение пакетов
  %(prog)s -p react -r https://registry.npmjs.org -f "dev" --mermaid
        """
    )
    ...
    parser.add_argument(
        '-m', '--mermaid',
        action='store_true',
        help='Вывести граф в формате Mermaid (Этап 5)'
    )
    ...
    parser.add_argument(
        ...
        metavar='FILE',
        default=None,
        help='Сохранить Mermaid диаграмму в файл'
    )
    
    parser.add_argument(
        '-v', '--version',
        action='version',
        version='%(prog)s 0.5.0 (Этапы 1-5)'
    )
...
def main() -> int:
    ...
    try:
        ...
        visualizer = DependencyVisualizer(
            ...
            output_mermaid=args.mermaid,
            mermaid_file=args.output
        )
